<x-layouts.page>
    <x-slot:banner>
        <section
        style="
          background-image: url('{{asset('img/services.jpg')}}');
          background-size: cover;
          background-attachment: fixed;
          background-position: center;
        "
        class="pt-12 min-h-96 relative"
      >
        <div
          class="absolute bottom-0 bg-gradient-to-t via-chil-blue/50 from-black/95 to-chil-red/30 h-full w-full"
        ></div>
        <div class="container mx-auto">
          <div class="absolute bottom-4" style="text-shadow: 3px 3px 8px black">
            <p
              data-aos="fade-down"
              data-aos-easing="linear"
              data-aos-duration="1500"
              class="max-w-2xl mb-6 font-light text-white lg:mb-1 md:text-lg lg:text-xl"
            >
              Services |
            </p>
            <h1
              data-aos="zoom-in"
              data-aos-easing="linear"
              data-aos-duration="1500"
              class="text-gray-100 max-w-2xl mb-4 text-2xl tracking-tight leading-none md:text-3xl"
            >
              Supported Living Service
            </h1>

            <div class="lg:mt-0 lg:col-span-5 lg:flex"></div>
          </div>
        </div>
      </section>
    </x-slot:banner>

    <main>

        <section class="container py-12 px-4 sm:px-6 lg:px-8">
            <!-- Header Section -->
            <header class="text-center">
              <h2
                data-aos="fade-up"
                data-aos-easing="ease-out-cubic"
                data-aos-duration="600"
                data-aos-delay="100"
                class="tracking-widest text-chil-red font-semibold uppercase"
              >
                Service Overview
              </h2>
              <h1
                data-aos="zoom-in"
                data-aos-easing="ease-out-cubic"
                data-aos-duration="1000"
                data-aos-delay="200"
                class="mb-4 text-2xl text-black-900 dark:text-white md:text-4xl 2xl:text-5xl mt-4 max-w-2xl text-gray-500 lg:mx-auto"
              >
                <span
                  class="text-transparent bg-clip-text bg-gradient-to-r to-red-600 from-blue-600"
                >
                  Supported Living, </span
                ><br />
                Empowering Independence with Personalized Support
              </h1>
            </header>

            <!-- Main Content Section -->
            <article class="mt-12 lg:flex lg:items-center">
              <!-- Image Section -->
              <figure
                data-aos="fade-up"
                data-aos-easing="ease-out-cubic"
                data-aos-duration="800"
                class="lg:w-1/2"
              >
                <img
                  class="rounded-lg shadow-lg"
                  src="{{asset('img/suport.jpg')}}"
                  alt="Supported Living Services"
                />
                <figcaption class="sr-only">Supported Living Service</figcaption>
              </figure>
              <!-- Description Section -->
              <div
                data-aos="fade-up"
                data-aos-easing="ease-out-cubic"
                data-aos-duration="800"
                data-aos-delay="300"
                class="lg:w-1/2 lg:pl-12 mt-6 lg:mt-0"
              >
                <h3 class="text-2xl leading-6 font-medium text-chil-blue">
                  What is Supported Living?
                </h3>
                <p class="mt-4 text-lg">
                  Supported Living provides tailored assistance to individuals who
                  need support with everyday activities, fostering greater
                  independence while remaining in a supportive environment. This
                  service is ideal for those who require help with personal care and
                  daily routines.
                </p>
                <h3 class="text-2xl leading-6 font-medium text-chil-blue mt-8">
                  Our Approach to Supported Living
                </h3>
                <ul class="mt-4 list-disc pl-5 space-y-2">
                  <li>
                    Customizable support plans to meet individual needs and
                    preferences.
                  </li>
                  <li>
                    Help with daily tasks such as personal hygiene, meal preparation,
                    and medication management.
                  </li>
                  <li>Support with household chores and maintenance.</li>
                  <li>
                    Social and recreational activities to enhance quality of life.
                  </li>
                  <li>Continuous monitoring and adjustment of care as needed.</li>
                </ul>
              </div>
            </article>

            <!-- Care Process Section -->
            <section
              data-aos="fade-up"
              data-aos-easing="ease-out-cubic"
              data-aos-duration="800"
              class="mt-12"
            >
              <h3 class="text-2xl leading-6 font-medium text-chil-blue">
                Our Care Process
              </h3>
              <p class="mt-4 text-lg text-gray-600">
                We offer a structured approach to Supported Living to ensure
                personalized and effective care:
              </p>
              <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mt-8">
                <div
                  data-aos="fade-up"
                  data-aos-easing="ease-out-cubic"
                  data-aos-duration="800"
                  class="flex items-start"
                >
                  <div class="flex-shrink-0">
                    <span
                      class="inline-flex items-center justify-center p-3 bg-chil-blue text-white rounded-md shadow-lg"
                    >
                      1
                    </span>
                  </div>
                  <div class="ml-4">
                    <h4 class="text-lg leading-6 font-medium text-chil-red">
                      Initial Consultation
                    </h4>
                    <p class="mt-2 text-base text-gray-600">
                      We start with a detailed consultation to understand the
                      individual's needs, preferences, and goals for their supported
                      living experience.
                    </p>
                  </div>
                </div>
                <div
                  data-aos="fade-up"
                  data-aos-easing="ease-out-cubic"
                  data-aos-duration="800"
                  data-aos-delay="200"
                  class="flex items-start"
                >
                  <div class="flex-shrink-0">
                    <span
                      class="inline-flex items-center justify-center p-3 bg-chil-blue text-white rounded-md shadow-lg"
                    >
                      2
                    </span>
                  </div>
                  <div class="ml-4">
                    <h4 class="text-lg leading-6 font-medium text-chil-red">
                      Personalized Support Plan
                    </h4>
                    <p class="mt-2 text-base text-gray-600">
                      We create a customized support plan that aligns with the
                      individual's lifestyle, health needs, and personal goals.
                    </p>
                  </div>
                </div>
                <div
                  data-aos="fade-up"
                  data-aos-easing="ease-out-cubic"
                  data-aos-duration="800"
                  data-aos-delay="400"
                  class="flex items-start"
                >
                  <div class="flex-shrink-0">
                    <span
                      class="inline-flex items-center justify-center p-3 bg-chil-blue text-white rounded-md shadow-lg"
                    >
                      3
                    </span>
                  </div>
                  <div class="ml-4">
                    <h4 class="text-lg leading-6 font-medium text-chil-red">
                      Ongoing Support and Review
                    </h4>
                    <p class="mt-2 text-base text-gray-600">
                      We offer ongoing support and regular reviews to ensure the care
                      plan remains effective and adjusts to any changes in the
                      individual's needs.
                    </p>
                  </div>
                </div>
              </div>
            </section>

            <!-- Testimonials Section -->
            <section
              data-aos="fade-up"
              data-aos-easing="ease-out-cubic"
              data-aos-duration="800"
              class="mt-12 bg-gray-50 py-8 px-6 rounded-lg shadow-md"
            >
              <h3 class="text-2xl leading-6 font-medium text-gray-900">
                What Our Clients Say
              </h3>
              <blockquote class="mt-6 space-y-4">
                <p class="text-lg text-gray-600 italic">
                  "The support from C.H.I.L has provided a much-needed balance of care
                  and independence for my sister's life. The support team is fantastic
                  and always attentive."
                </p>
                <footer class="text-right text-sm font-medium text-gray-900">
                  — E.W, Client's Sister
                </footer>

                <p class="text-lg text-gray-600 italic">
                  "The support has made a huge difference in my loved one’s quality of
                  life. They feel more secure and self-sufficient."
                </p>
                <footer class="text-right text-sm font-medium text-gray-900">
                  — D.E, Client's Family Member
                </footer>
              </blockquote>
            </section>

            <!-- FAQ Section -->
            <section
              data-aos="fade-up"
              data-aos-easing="ease-out-cubic"
              data-aos-duration="800"
              class="mt-12"
            >
              <h3 class="text-2xl leading-6 font-medium text-gray-900">
                Frequently Asked Questions
              </h3>
              <dl class="mt-6">
                <div class="py-6">
                  <dt class="text-lg leading-6 font-medium text-gray-900">
                    What does Supported Living include?
                  </dt>
                  <dd class="mt-2 text-base text-gray-600">
                    Supported Living includes assistance with daily activities,
                    personal care, household tasks, and social activities, all
                    tailored to the individual's needs.
                  </dd>
                </div>
                <div class="py-6">
                  <dt class="text-lg leading-6 font-medium text-gray-900">
                    How flexible is the Supported Living schedule?
                  </dt>
                  <dd class="mt-2 text-base text-gray-600">
                    Supported Living schedules are highly adaptable. We offer flexible
                    arrangements to suit the client’s requirements, from part-time
                    support to full-time care.
                  </dd>
                </div>
              </dl>
            </section>

            <!-- Contact Section -->
            <aside
              data-aos="fade-up"
              data-aos-easing="ease-out-cubic"
              data-aos-duration="800"
              class="mt-12 text-center"
            >
              <p class="text-lg text-gray-600">
                If you or someone you know would benefit from Supported Living, please
                reach out to us for more information.
              </p>
              <div class="mt-4">
                <a
                  href="/contact.html"
                  class="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-chil-red/90 hover:bg-chil-blue"
                >
                  Contact Us Today
                </a>
              </div>
            </aside>
          </section>
          <x-hr />

          <x-services />

          <x-hr />

<x-about />
    <hr />

    <x-contact />
    </main>
</x-layouts.page>
